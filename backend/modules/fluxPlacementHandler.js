...
console.log('FLUX_HANDLER_VERSION: 2025-06-12_V1.11_FINAL_MASK_CONFIRMATION_RETEST'); // UPDATED VERSION LOG - Re-enabling standard mask inversion
...
     * @param {string} maskBase64 The base64 string of the mask (white area is tattoo).
     * @returns {Promise<string>} A promise that resolves with the inverted base64 mask (black area is tattoo).
...
        
        // 2. Handle mask:
        // Frontend draws WHITE where we want the tattoo, and BLACK elsewhere.
...
        // THEREFORE, we need to INVERT it for Flux.
        let processedMaskBase64;
        try {
            processedMaskBase64 = await fluxPlacementHandler.invertMask(maskBase64); // RE-ENABLE INVERSION HERE
            console.log('Mask inversion performed for Flux API: frontend white area -> backend black mask.');
        } catch (error) {
...
        const effectivePrompt = `Integrate this uploaded tattoo design (from reference image) onto the human skin realistically and seamlessly within the masked area, preserving the original skin outside the mask. Ensure the tattoo matches the reference design exactly. Realistic photo, professional tattoo photography, high detail. ${userPrompt ? 'Additional instructions: ' + userPrompt : ''}`; // REFINED PROMPT
...
            mask_image: processedMaskBase64, // Send INVERTED mask (black = tattoo area)
            reference_images: [tattooDesignBuffer.toString('base64')], // Pass the tattoo design here
            n: numVariations, // Request 3 variations
            output_format: 'jpeg',
            fidelity: 1.0, // HIGHEST fidelity to ensure tattoo design is present
            guidance_scale: 12.0, // Stronger adherence to the prompt
...
        console.log('Calling Flux Kontext API with payload (truncated images for log):');
        ...
        debugPayload.reference_images = debugPayload.reference_images && debugPayload.reference_images.length > 0 ? debugPayload.reference_images[0].substring(0, 50) + '...' : 'N/A';
